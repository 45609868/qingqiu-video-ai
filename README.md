...
清秋AI短剧 是一个基于 AI 的短剧自动化生产平台，实现从剧本生成、角色设计、分镜制作到视频合成的全流程自动化。

清秋AI短剧商业版地址：[清秋AI短剧商业版](https://qingqi.ai/shortvideo)
...
```bash
# 克隆项目
git clone https://github.com/qingqi-ai/qingqiu-drama.git
cd qingqiu-drama
...
数据库表在首次启动时自动创建，无需手动迁移。默认路径 `data/qingqiu_drama.db`，可通过环境变量覆盖：
...
docker run -d \
  ...
  qingqiu/qingqiu-drama:latest

# 查看日志
docker logs -f qingqiu-drama
```
...
```bash
docker build -t qingqiu-drama:latest .
docker run -d --name qingqiu-drama -p 5679:5679 \
  -v $(pwd)/data:/app/data \
  -v $(pwd)/configs/config.yaml:/app/configs/config.yaml \
  qingqiu-drama:latest
```
...
- 🐙 **GitHub**: [https://github.com/qingqi-ai/qingqiu-drama](https://github.com/qingqi-ai/qingqiu-drama)
...
[![Star History Chart](https://api.star-history.com/svg?repos=qingqi-ai/qingqiu-drama&type=date&legend=top-left)](https://www.star-history.com/#qingqi-ai/qingqiu-drama&type=date&legend=top-left)
Made with ❤️ by 清秋AI
