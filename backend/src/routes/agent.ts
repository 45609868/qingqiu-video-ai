...
app.post('/:type/chat', async (c) => {
  ...
  try {
    ...
    let editorResult: any = null

    if (agentType === 'script_rewriter') {
      const editor = createAgent('story_editor', episode_id, drama_id)
      if (editor) {
        logTaskStart('Agent', 'story_editor:auto-before-script_rewriter', {
          dramaId: drama_id,
          episodeId: episode_id,
        })
        editorResult = await editor.generate(
          [{
            role: 'user',
            content: '请先作为短剧总编，分析当前集的核心卖点、取舍策略、推荐结构，并输出给 script_rewriter 的执行指令。',
          }],
          { maxSteps: 10 },
        )
        logTaskSuccess('Agent', 'story_editor:auto-before-script_rewriter', {
          textLength: editorResult.text?.length || 0,
        })
        finalMessage = [
          message,
          '',
          '以下是短剧总编 story_editor 已经完成的改写策略。你必须优先按这个策略执行，但仍需遵守自身剧本格式、剧情边界和 save_script 保存要求：',
          '',
          editorResult.text || '',
        ].join('\n')
        logTaskPayload('Agent', 'story_editor auto strategy', {
    ...
    return success(c, {
      type: 'done',
      text: result.text || '',
      storyEditor: editorResult ? {
        text: editorResult.text || '',
        toolCalls: (editorResult.toolCalls || []).map((tc: any) => ({
          toolName: normalizeToolName(tc),
          args: tc?.args ?? tc?.input ?? null,
        })),
        toolResults: (editorResult.toolResults || []).map((tr: any) => ({
          toolName: normalizeToolName(tr),
          result: normalizeToolResult(tr),
        })),
      } : null,
