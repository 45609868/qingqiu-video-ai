...
function assertActiveServiceConfig(id: number, serviceType: string) {
  const [row] = db.select().from(schema.aiServiceConfigs)
    .where(eq(schema.aiServiceConfigs.id, id)).all()
  if (!row || !row.isActive || row.serviceType !== serviceType) {
    throw new Error(`${serviceType}_config_id must point to an active ${serviceType} config`)
  }
}
...
app.delete('/:id', async (c) => {
  ...
  db.update(schema.episodes)
    .set({ deletedAt: nowStr, updatedAt: nowStr })
...
app.put('/:id', async (c) => {
  ...
  try {
    if ('image_config_id' in updates) assertActiveServiceConfig(Number(updates.image_config_id), 'image')
    if ('video_config_id' in updates) assertActiveServiceConfig(Number(updates.video_config_id), 'video')
    if ('audio_config_id' in updates) assertActiveServiceConfig(Number(updates.audio_config_id), 'audio')
  } catch (err: any) {
