...
function buildGridCellPrompts(
  ...
  if (mode === 'multi_ref') {
    ...
    return Array.from({ length: rows * cols }, (_, i) => ({
      ...
      prompt: `${cellLabel(i, rows, cols)}: ${buildStoryboardReferenceHints(sb, referenceAssets, storyboardCharacterIds).join('、')}${buildStoryboardReferenceHints(sb, referenceAssets, storyboardCharacterIds).length ? '，' : ''}${desc}, ${angles[i % angles.length]}, ${style} style`,
    }))
  }
  ...
  if (mode === 'first_last') {
    return Array.from({ length: rows * cols }, (_, i) => {
      ...
      return {
        ...
        prompt: isFirst
          ? `${cellLabel(i, rows, cols)}，首帧：${refs.length ? `参考${refs.join('、')}，` : ''}${desc}${sb.location ? `, ${sb.location}` : ''}${sb.shotType ? `, ${sb.shotType}` : ''}, ${style} style`
          : `${cellLabel(i, rows, cols)}，尾帧：${refs.length ? `参考${refs.join('、')}，` : ''}${desc}${motion ? `, ${motion}` : ''}${sb.location ? `, ${sb.location}` : ''}${sb.shotType ? `, ${sb.shotType}` : ''}, ${style} style`,
      }
    })
  }
  ...
  return storyboards.slice(0, rows * cols).map((sb, index) => {
    ...
    return {
      ...
      prompt: `${cellLabel(index, rows, cols)}：${refs.length ? `参考${refs.join('、')}，` : ''}${desc}${sb.location ? `, ${sb.location}` : ''}${sb.shotType ? `, ${sb.shotType}` : ''}, opening scene, ${style} style`,
    }
  })
}
...
app.post('/generate', async (c) => {
  ...
  // Size: 9:16 vertical format for short drama video
  // Cell: 540x960 (9:16). For NxM grid, total = (540N)x(960M)
  // For 2x3 grid: 1080x2880 → still 9:16 ✓
  const cellW = 540, cellH = 960
  const actualCols = cols
  const actualRows = rows
