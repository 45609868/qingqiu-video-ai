...
app.put('/:id', async (c) => {
  ...
  for (const key of ['name', 'role', 'description', 'appearance', 'personality', 'voiceStyle', 'voiceProvider', 'imageUrl', 'lockedImageUrl', 'imageLockStatus', 'referencePrompt', 'localPath']) {
...
app.post('/:id/generate-image', async (c) => {
  ...
  try {
    logTaskStart('CharacterImage', 'generate', { characterId: id, episodeId: ep.id, dramaId: char.dramaId, dramaStyle })
    const genId = await generateImage({ characterId: id, dramaId: char.dramaId, prompt, size: '1080x1920', configId: ep.imageConfigId ?? undefined })
