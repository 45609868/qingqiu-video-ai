...
## 6. 台词规则
- 每句台词 ≤ 20字（硬性限制，超出必须拆分）
- 口语化：像演员能直接念出来的台词，不是叙述
- 禁止"XXX想到"、"XXX感觉到"等小说第三人称心理描写
- 强制转为可表演的台词+动作描述

## 7. 冲突触发规则
- 每3句台词内必须触发一次小冲突/情绪变化
- 小冲突类型：质问、威胁、嘲讽、揭穿、拒绝、哭诉、崩溃
- 情绪变化类型：冷笑→震怒、惊讶→恐惧、沉默→爆发
- 禁止连续3句以上平稳对话（无冲突无情绪）

## 8. 开场钩子规则（前3秒）
- 开篇第1句话必须是强冲突或强悬念
- 禁止慢铺垫、禁止背景介绍开头
- 可用：突然闯入、身份揭穿、危险逼近、质问、威胁、尖叫
- 示例：
  ❌ 弱钩子：男主回到家中，发现门开着...
  ✅ 强钩子："你凭什么动我的东西！"她尖叫着冲进来。

## 9. 结构节奏模板
标准120-180秒短剧结构：
- S01（开场）：前3秒强钩子，抛出核心冲突
- S02-S03（推进）：围绕核心冲突展开，2个镜头推进
- S04（小高潮）：情绪爆发点，信息揭晓
- S05（转折）：意外事件或身份反转
- S06-S07（收尾）：留下悬念，吸引继续看

## 10. 风格适配（根据 drama.style 自动选择）

【anime / 玄幻风格】
- 法术展示优先：镜头优先展示法术效果、能量波动
- 动感运镜：推拉镜头、旋转、快速剪辑
- 世界观暗示：用场景和特效暗示世界观，不直接说明
- 对白简洁有力度，动作描写比心理描写多
- 色彩饱和度高，光影强烈
...
【职位/昵称统一】
- "X总" + "X全名" → 识别为同一人（通过职位判断）
- "X哥" + "X全名" → 识别为同一人（通过称呼判断）
- "X小姐" + "X全名" → 识别为同一人

【别名列表】
- 每个角色需记录所有别名，用于后续去重
- 输出格式：name: 主名, aliases: [别名列表]

## 2. 角色外貌特征标准化
提取的外貌描述必须包含以下维度（用于文生图一致性锚点）：
...
【禁止】模糊描述如"很漂亮"、"气质好"
【必须】具体可画的描述如"冷白皮肤、黑色长发、红色长裙"

## 3. 角色 reference_prompt 生成
每个主角必须生成 reference_prompt，用于后续角色图生成的一致性锚点：
...
- 重要配角：多集出现，推动剧情
- 龙套：单集出现，服务单一功能
- 仅提及：名字出现但未实际出场，只记录信息不生成复杂形象

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【场景提取核心规则】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

## 5. 场景描述标准化（适配文生图 prompt）
提取的场景描述必须去除抽象词，转化为可生成的画面描述：

【禁止】抽象词：
- "金碧辉煌" → 改为"豪华宫殿，暖黄灯光，大理石地面"
- "阴森恐怖" → 改为"昏暗走廊，冷色灯光，斑驳墙壁"
- "浪漫氛围" → 改为"海边日落，暖橙光线，薄纱窗帘"
...
## 1. 镜头类型自动分配规则
根据台词长度和内容类型自动选择最合适的景别：

【台词长 + 情绪强 → 近景/特写】
- 台词超过15字、或情绪高潮点
- 目的：让观众看清演员表情变化
- 示例：对白质问你凭什么、揭露真相、情绪崩溃
...
【fade_in / fade_out】
- 适用：开场、结尾、时间跨度大的过渡
- FFmpeg：xfade=transition=fade:duration=0.5
...
【blur / 模糊转场】
- 适用：梦境、回忆、情绪过渡
- FFmpeg：boxblur=xfade

【dissolve / 叠化】
- 适用：时间流逝、身份转换
- FFmpeg：xfade=transition=dissolve
...
每个镜头必须尽量完整填写以下字段：
- title：3-8 字镜头标题
- shot_type：景别，如全景/中景/近景/特写/极端特写
...
- drama_function：戏剧功能，如推进/冲突/反转/情绪爆点/信息传递
- transition：转场方式，如 fade_in/fade_out/cut/blur/dissolve/slap_head
...
- 每个镜头是否都有 shot_type / movement / drama_function / transition 字段
- 是否遵守景别多样性规则（无连续3个以上相同景别）
