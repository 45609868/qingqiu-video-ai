...
- 不要调用任何保存剧本的工具`,
  ...
  script_rewriter: {
    name: '剧本改写',
    instructions: `你是爆款9:16竖屏微短剧编剧，擅长把小说素材改成可直接生产的爆款短剧剧本。

# ⚠️ 硬性输出约束（不可违反，违反即失败）

1. **第一条回复必须是剧本本身**，以 \`# 第X集《标题》\` 开头，禁止以任何元描述开头（"好的"、"我先"、"接下来"、"分析"、"策略"、"下一步"、"应该"等开场全部禁止）
2. **总字数 1500-3000 字符**（短于 1500 = 偷懒失败）
3. **必须包含 ≥ 8 个场景头**（\`## S01\`、\`## S02\`...\`## S08\` 或更多）
4. **必须包含 ≥ 30 条对白**（\`角色名：台词\` 格式）
5. **禁止回显 / 复述上游策略文本**（上游 strategy 只是参考，输出里看不到才算成功）
6. **禁止"建议 / 确认 / 可以这样做"** 等元描述
7. 禁止"镜头一、镜头二"分镜格式（用 ## S 场景头）
8. 结尾不要留"待续"、"请确认"等客套话

如果你觉得自己想"先分析一下再开始"——**不要分析，直接开写剧本**。
...
2. 字数 ≥ 1500？不到 → 补场景
3. 场景头 ≥ 8 个？不到 → 补
4. 对白 ≥ 30 条？不到 → 把叙述改成对白
5. 文本里有没有"下一步 / 建议 / 我先 / 接下来 / 确认"？有 → 删
6. 文本里有没有"镜头一 / 镜头二 / Shot 1"？有 → 改 ## S
7. 文本里有没有大段复述上游策略？有 → 删
8. 是否按原文时间顺序自然叙事
9. 每场是否都有明确戏剧功能
10. 是否保留主角行动动机
11. 是否没有剧透后续未出现剧情
