...
export function createStoryboardTools(episodeId: number, dramaId: number) {
  const readStoryboardContext = createTool({
    ...
    execute: async () => {
      ...
      // 强制只读 script_content，不回退到 content
      if (!ep.scriptContent) return { error: 'Episode has no script_content. Run script_rewriter first.' }
      const script = ep.scriptContent
      logTaskProgress('StoryboardTool', 'read-context', {
        episodeId,
        contentLength: ep.content?.length || 0,
        scriptContentLength: ep.scriptContent.length,
        scriptPreview: ep.scriptContent.slice(0, 200),
      })
      ...
      logTaskSuccess('StoryboardTool', 'read-context', {
        ...
        dramaStyle: payload.drama_style,
