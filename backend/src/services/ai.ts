...
export function getAudioConfigById(id?: number | null): AIConfig {
  if (id) {
    const config = getConfigById(id, 'audio')
    if (config) return config
  }
  return getAudioConfig()
}

export function getConfigById(id: number, expectedServiceType?: ServiceType): AIConfig | null {
  ...
  if (expectedServiceType && row.serviceType !== expectedServiceType) {
    logTaskWarn('AIConfig', 'config-by-id-service-mismatch', {
      configId: id,
      expectedServiceType,
