...
export async function generateVideo(params: GenerateVideoParams): Promise<number> {
  ...
  const lockedConfig = params.configId
    ? getConfigById(params.configId, 'video')
    : null
  if (params.configId && !lockedConfig) {
    logTaskWarn('VideoTask', 'locked-config-fallback', { configId: params.configId, fallback: 'active-video-config' })
  }
  const config = lockedConfig || getActiveConfig('video')
  ...
  for (let i = 0; i < 300; i++) {
    ...
    try {
      ...
      if (pollResp.status === 'failed') {
        const errorMsg = stringifyError(pollResp.error || 'Video generation failed')
      ...
      logTaskWarn('VideoTask', 'poll-retry', { id, taskId, attempt: i + 1, error: stringifyError(err) })
    }
  }
}
