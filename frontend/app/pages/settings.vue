        ...
        <div class="settings-head">
          <div class="settings-brand">
            <div class="settings-brand-mark">
              <img v-if="showBrandImage" :src="brandLogo" alt="清秋AI短剧" class="settings-brand-logo" @error="showBrandImage = false" />
              <span v-else class="settings-brand-fallback">清</span>
            </div>
            <div class="settings-brand-copy">
              <div class="settings-brand-kicker">qingqi.ai</div>
              <div class="settings-brand-name">清秋AI短剧</div>
            </div>
          </div>
        ...
        <div class="sections">
          <section v-for="st in serviceTypes" :key="st.type">
            <div class="section-head">
              ...
              <button v-if="st.type === 'audio'" class="btn btn-ghost btn-sm" :disabled="syncingVoices" @click="syncVoices">
                <Loader2 v-if="syncingVoices" :size="12" class="animate-spin" />
                {{ syncingVoices ? '同步中...' : '同步音色' }}
              </button>
                ...
                <div class="config-info">
                  <div class="config-main">
                    <div class="config-line">
                      <span class="config-provider">{{ PROVIDER_LABELS[c.provider] || c.provider }}</span>
    ...
    <div v-if="cfgDialog" class="overlay" @click.self="cfgDialog = false">
      <form class="modal card config-modal" @submit.prevent="saveCfg">
        ...
        <label class="field">
          ...
          <input v-model="cfgForm.name" class="input" placeholder="如 清秋默认图像服务" />
        </label>
        <label class="field"><span class="field-label">服务商</span>
          <BaseSelect v-model="cfgForm.provider" :options="PROVIDER_SELECT_OPTIONS" placeholder="选择服务商" searchable />
        </label>
        <label class="field">
  ...
  <form class="modal card config-modal" @submit.prevent="applyHuobaoPreset">
    <div class="config-modal-head">
      <div>
        ...
        <h2 class="modal-title">清秋一键配置</h2>
        <div class="modal-note">按清秋推荐链路自动创建或更新 4 条服务配置，并同时初始化 5 个 Agent 的默认模型。</div>
          </div>
          <span class="tag tag-success">推荐</span>
        </div>
        <div class="qingqiu-grid">
          <label class="field">
            <span class="field-label">清秋 API Key <span class="dim">(统一用于文本 / 图片 / 视频 / 音频)</span></span>
            <input v-model="huobaoForm.apiKey" class="input" type="password" placeholder="用于清秋AI短剧全链路服务" />
            <span class="field-hint">还没有账号？<a href="http://localhost:3000/" target="_blank" rel="noopener">立即注册 →</a></span>
          </label>
        </div>
...
const providerPresets = {
  text: {
    openai: { label: 'OpenAI 推荐', baseUrl: 'http://localhost:3000', models: ['gpt-5.5'] },
    chatfire: { label: 'ChatFire 推荐', baseUrl: 'http://localhost:3000', models: ['gpt-5.5'] },
    openrouter: { label: 'OpenRouter 推荐', baseUrl: 'https://openrouter.ai/api', models: ['google/gemini-3-flash-preview'] },
  },
  image: {
    openai: { label: 'OpenAI 推荐', baseUrl: 'http://localhost:3000', models: ['gpt-image-2'] },
    chatfire: { label: 'ChatFire 推荐', baseUrl: 'http://localhost:3000', models: ['gpt-image-2'] },
    gemini: { label: 'Gemini 推荐', baseUrl: 'http://localhost:3000', models: ['gpt-image-2'] },
  ...
  audio: {
    minimax: { label: 'MiniMax 推荐', baseUrl: 'http://localhost:3000/minimax', models: ['speech-2.8-hd'] },
  },
}
const huobaoPresetCards = [
  { serviceType: 'text', label: '文本', provider: 'openai', baseUrl: 'http://localhost:3000', model: 'gpt-5.5', priority: 100 },
  { serviceType: 'image', label: '图片', provider: 'openai', baseUrl: 'http://localhost:3000', model: 'gpt-image-2', priority: 99 },
  { serviceType: 'video', label: '视频', provider: 'volcengine', baseUrl: 'http://localhost:3000', model: 'doubao-seedance-1-5-pro-251215', priority: 98 },
  { serviceType: 'audio', label: '音频', provider: 'minimax', baseUrl: 'http://localhost:3000/minimax', model: 'speech-2.8-hd', priority: 97 },
]
...
const endpointHint = computed(() => {
  ...
  // 如果 baseUrl 已经包含了该 provider 的典型路径后缀，不再追加 prefix
  if (prefix && base !== 'https://...' && (base.endsWith(prefix) || base.includes('/video') || base.includes('/v1/' + provider))) {
    return base
  }
  return `${base}${prefix}`
})
...
async function syncVoices() {
  ...
  try {
    const res = await fetch('/api/v1/ai-voices/sync', { method: 'POST' })
    const json = await res.json()
    if (json.code === 200) toast.success(`已同步 ${json.data.count} 个音色`)
    else toast.error(json.message || '同步失败')
  } catch (e) { toast.error(e.message) }
  finally { syncingVoices.value = false }
}
...
async function applyHuobaoPreset() {
  ...
  try {
    ...
    toast.success('清秋推荐配置与默认 Agent LLM 已写入')
  } catch (e) {
    toast.error(e.message)
  }
}
...
const defaultPrompts = {
  script_rewriter: `你是电影级竖屏短剧编剧，擅长将小说改编为可直接生产的一集短剧剧本。

工作流程：
1. 调用 read_episode_script 读取原始内容
2. 提炼本集最强冲突、反转和结尾钩子，不要照搬小说流水账
3. 调用 save_script 保存改写后的完整剧本

短剧剧本目标：
- 第一集成片时长优先控制在 120-180 秒
- 前 3-5 秒必须出现强钩子或异常事件
- 保留主线冲突，压缩铺垫和解释性旁白
- 对白短、狠、可表演，旁白只用于跳过不适合拍的过场

格式化剧本格式：
- 场景头：## S编号 | 内景/外景 · 地点 | 时间段
- 戏剧功能：钩子 / 误会 / 反转 / 危机 / 悬疑 / 情绪爆点 / 结尾钩子
- 动作描写：只写可拍到的动作、表情、道具和环境
- 对白：角色名：（情绪/动作）台词内容；多角色对话必须分行
- 旁白：必要信息压缩，每句尽量不超过 20 字
- 结尾钩子：留下下一步悬念或危机`,
...
提取要求：
- 只提取当前集真实出现、说话、被镜头表现、或对本集冲突有效的角色和场景
- 不要遗漏“某某的声音/呼救声/怒吼/写道/说道”这类间接出场角色
- 区分角色等级：主角 / 重要配角 / 龙套 / 仅提及
- 角色要包含完整的外貌特征描述（发型、服装、体态等）和短剧表演定位
- 场景要包含光线、色调、氛围等视觉信息
- 同一大场景的不同区域要保持关系
- 不要遗漏任何有台词或重要动作的角色`,
  storyboard_breaker: `你是电影级竖屏短剧分镜导演，目标是把剧本拆成可直接生成视频的一集短剧。

工作流程：
1. 调用 read_storyboard_context 读取剧本、角色列表、场景列表
2. 先判断本集核心钩子、反转和结尾悬念
3. 拆成 12-15 个核心镜头，整体时长优先 90-180 秒；不要生成小说流水账
4. 为每个镜头补全 title、景别、机位、动作、对白、首帧画面、尾帧结果、图片提示词、视频提示词
...
.qingqiu-grid .field-hint a {
  color: var(--accent);
  text-decoration: none;
  font-weight: 500;
}
.qingqiu-grid .field-hint a:hover {
