<template>
  <div class="studio" v-if="drama">
    <header class="studio-topbar">
      ...
      <div class="studio-topbar-side">
        <div class="episode-config-strip">
          <label class="episode-config-item">
            <span>图片</span>
            <BaseSelect
              :model-value="lockedImageConfigId"
              :options="imageConfigSelectOptions"
              placeholder="图片配置"
              searchable
              style="width:180px"
              @update:model-value="updateEpisodeConfig('image_config_id', $event)"
            />
          </label>
          <label class="episode-config-item">
            <span>视频</span>
            <BaseSelect
              ...
              searchable
              style="width:180px"
              @update:model-value="updateEpisodeConfig('video_config_id', $event)"
            />
          </label>
          <label class="episode-config-item">
            <span>音频</span>
            <BaseSelect
              ...
              searchable
              style="width:180px"
              @update:model-value="updateEpisodeConfig('audio_config_id', $event)"
            />
          </label>
        </div>
    ...
    <main class="main">
        ...
        <div v-if="scriptStep === 0" class="step-editor">
          <div class="step-toolbar">
            ...
            <div class="toolbar-right">
              ...
              >
                <Loader2 v-if="isGenerating" :size="11" class="animate-spin" />
      ...
      <div v-else-if="panel === 'production'" class="content-panel">
        ...
        <template v-else>
          ...
          <!-- Sub: Health -->
          <div v-if="prodTab === 'health'" class="prod-content">
            <div class="prod-section-bar">
              <span class="dim" style="font-size:12px">制作链路体检</span>
              <span class="tag" :class="(healthData?.summary?.errors || 0) ? 'tag-danger' : 'tag-success'">错误 {{ healthData?.summary?.errors || 0 }}</span>
              <span class="tag">提醒 {{ healthData?.summary?.warnings || 0 }}</span>
              <button class="btn btn-sm ml-auto" :disabled="dashboardLoading" @click="loadDashboard">刷新体检</button>
            </div>
            <div class="health-list">
              <div v-for="item in (healthData?.items || [])" :key="`${item.area}-${item.title}-${item.detail}`" class="health-row" :class="`is-${item.level}`">
                <div class="health-level">{{ healthLevelLabel(item.level) }}</div>
                <div class="health-copy">
                  <div class="health-title">{{ item.area }} · {{ item.title }}</div>
                  <div class="health-detail">{{ item.detail }}</div>
                </div>
                <span v-if="item.action" class="tag">{{ item.action }}</span>
              </div>
              <div v-if="!(healthData?.items || []).length" class="step-empty" style="min-height:240px">
                <div class="empty-title">还没有体检结果</div>
                <div class="empty-desc">点击刷新体检，检查模型配置、角色/场景标准图、镜头绑定和失败任务。</div>
              </div>
            </div>
          </div>

          <!-- Sub: Tasks -->
          <div v-else-if="prodTab === 'tasks'" class="prod-content">
            <div class="prod-section-bar">
              <span class="dim" style="font-size:12px">最近生成任务</span>
              <span class="tag">总计 {{ tasksData?.summary?.total || 0 }}</span>
              <span class="tag" :class="failedTaskCount ? 'tag-danger' : ''">失败 {{ failedTaskCount }}</span>
              <button class="btn btn-sm ml-auto" :disabled="dashboardLoading" @click="loadDashboard">刷新任务</button>
            </div>
            <div class="task-list">
              <div v-for="task in (tasksData?.items || [])" :key="task.id" class="task-row">
                <div class="task-main">
                  <div class="task-title">
                    <span class="frame-badge">{{ taskKindLabel(task.kind) }}</span>
                    <span>{{ task.target_label }}</span>
                  </div>
                  <div class="task-prompt">{{ task.prompt || task.error_msg || '无提示词记录' }}</div>
                  <div v-if="task.error_msg" class="prod-error">{{ task.error_msg }}</div>
                </div>
                <div class="task-side">
                  <span class="tag" :class="task.status === 'failed' ? 'tag-danger' : (task.status === 'completed' ? 'tag-success' : '')">{{ taskStatusLabel(task.status) }}</span>
                  <span class="dim">{{ task.provider || '—' }}</span>
                </div>
              </div>
              <div v-if="!(tasksData?.items || []).length" class="step-empty" style="min-height:240px">
                <div class="empty-title">暂无任务记录</div>
                <div class="empty-desc">生成角色、场景、镜头图片或视频后，这里会集中展示状态和失败原因。</div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="prodTab === 'chars'" class="prod-content">
            ...
            <div class="asset-grid">
              <div v-for="c in visualChars" :key="c.id" class="card asset-card">
                <div class="asset-cover character">
                  <img
                ...
                <div class="asset-body">
                  ...
                  <textarea
                    class="mini-textarea"
                    rows="3"
                    :value="getReferencePrompt(c) || buildDefaultCharReferencePrompt(c)"
                    placeholder="固定角色描述：脸型、发型、年龄、服装、体型..."
                    @blur="saveCharReferencePrompt(c, $event.target.value)"
                  />
                </div>
                <div class="asset-foot">
                  <span :class="['dot', (c.image_url || c.imageUrl) && 'ok', isPendingCharImage(c.id) && 'pending']" />
                  <span class="dim" style="font-size:10px">{{ (c.image_url || c.imageUrl) ? '标准图' : (isPendingCharImage(c.id) ? '生成中' : '待生成') }}</span>
                  <button class="btn btn-sm ml-auto" @click="uploadCharImg(c)">上传</button>
              ...
              <div v-for="s in scenes" :key="s.id" class="card asset-card">
                ...
                <div class="asset-body">
                  ...
                  <textarea
                    class="mini-textarea"
                    rows="3"
                    :value="getReferencePrompt(s) || buildDefaultSceneReferencePrompt(s)"
                    placeholder="固定场景描述：布局、建筑、光线、时代、色彩..."
                    @blur="saveSceneReferencePrompt(s, $event.target.value)"
                  />
                </div>
                <div class="asset-foot">
                  <span :class="['dot', (s.image_url || s.imageUrl) && 'ok', isPendingSceneImage(s.id) && 'pending']" />
                  <span class="dim" style="font-size:10px">{{ (s.image_url || s.imageUrl) ? '标准图' : (isPendingSceneImage(s.id) ? '生成中' : '待生成') }}</span>
                  <button class="btn btn-sm ml-auto" @click="uploadSceneImg(s)">上传</button>
                  <button class="btn btn-sm" :disabled="isPendingSceneImage(s.id)" @click="genSceneImg(s.id)">{{ isPendingSceneImage(s.id) ? '生成中' : '生成' }}</button>
                  <button v-if="s.image_url || s.imageUrl" class="btn btn-sm" style="color:var(--error);margin-left:4px" @click="deleteSceneImg(s)">
          ...
          <div v-else-if="prodTab === 'shots'" class="prod-content">
            <div class="prod-section-bar">
              ...
              <span class="tag mono">首 {{ firstFrameCount }}/{{ sbs.length }}</span>
              <span v-if="frameMode === 'first_last'" class="tag mono">尾 {{ lastFrameCount }}/{{ sbs.length }}</span>
              <span class="tag">{{ lockedImageConfigLabel }}</span>
              <div class="ml-auto flex gap-1">
                <BaseSelect v-model="frameMode" :options="frameModeOptions" placeholder="帧模式" searchable style="width:100px" />
                <button class="btn btn-sm" :disabled="pendingBatchShotFrames" @click="batchGenerateShotFrames('first')">
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 5v14"/><path d="M5 12h14"/></svg>
                  补首帧
                </button>
                <button v-if="frameMode === 'first_last'" class="btn btn-sm" :disabled="pendingBatchShotFrames" @click="batchGenerateShotFrames('last')">
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 5v14"/><path d="M5 12h14"/></svg>
                  补尾帧
                </button>
                <button class="btn btn-primary btn-sm" :disabled="pendingBatchShotFrames" @click="batchGenerateShotFrames('missing')">
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M20 6 9 17l-5-5"/></svg>
                  一键补缺失
                </button>
                ...
                <div v-for="(sb, i) in sbs" :key="sb.id"
                  ...
                  <div class="frame-info">
                    ...
                    <div class="frame-ref-strip">
                      <span
                        v-for="ref in getShotReferenceSummary(sb)"
                        :key="ref.key"
                        :class="['frame-ref-chip', ref.ok ? 'ok' : 'missing']"
                        :title="ref.tip"
                      >{{ ref.label }}</span>
                    </div>
                  </div>
                  ...
                  <div class="frame-thumbs">
                    <div class="frame-thumb-wrap">
                      ...
                      <button class="frame-thumb-label frame-upload-btn" @click.stop="uploadShotFrame(sb, 'first_frame')">{{ isPendingShotFrame(sb.id, 'first_frame') ? '首帧生成中' : '上传首帧' }}</button>
                    </div>
                    <div v-if="frameMode === 'first_last'" class="frame-thumb-wrap">
                      ...
                      <button class="frame-thumb-label frame-upload-btn" @click.stop="uploadShotFrame(sb, 'last_frame')">{{ isPendingShotFrame(sb.id, 'last_frame') ? '尾帧生成中' : '上传尾帧' }}</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="prod-grid">
              <div v-for="(sb, i) in sbs" :key="sb.id" class="card prod-card">
                ...
                <div class="prod-actions">
                  <button class="btn btn-sm" @click="openShotRefs(sb)">参考图</button>
                  <button v-if="videoFailMessage(sb.id)" class="btn btn-sm" :disabled="isPendingVideo(sb.id)" @click="retrySafeVideo(sb)">安全重试</button>
            ...
            <div class="prod-grid">
              <div v-for="(sb, i) in sbs" :key="sb.id" class="card prod-card">
                <div class="prod-cover">
                  ...
                  <span class="prod-idx">#{{ String(i+1).padStart(2,'0') }}</span>
                  <span v-if="hasComposed(sb)" class="prod-overlay-badge">配音+字幕版</span>
                </div>
                <div class="prod-info">
                  <div class="prod-desc truncate">{{ sb.description || sb.title || '—' }}</div>
                  <div class="prod-meta-line">{{ sb.shot_type || sb.shotType || '未设景别' }} · {{ sb.duration || 10 }}s</div>
                  <div class="prod-dots">
                    <span :class="['dot', hasVid(sb) && 'ok']" /><span style="font-size:10px">{{ hasComposed(sb) ? '原始' : '原始视频' }}</span>
                    <span :class="['dot', hasTTS(sb) && 'ok']" /><span style="font-size:10px">配音</span>
                    <span :class="['dot', hasComposed(sb) && 'ok', isPendingCompose(sb.id) && 'pending']" /><span style="font-size:10px">{{ isPendingCompose(sb.id) ? '合成中' : (hasComposed(sb) ? '配音+字幕' : '待合成') }}</span>
                  </div>
      ...
      <div v-if="refPreview.open" class="overlay" @click.self="closeRefPreview">
        <div class="card ref-preview-dialog">
          <div class="image-viewer-head">
            <div class="image-viewer-title">镜头参考图</div>
            <button class="btn btn-ghost btn-icon" @click="closeRefPreview">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </div>
          <div class="ref-preview-grid">
            <div v-for="ref in refPreview.refs" :key="ref.key" class="ref-preview-item">
              <img v-if="ref.src" :src="'/' + ref.src" class="ref-preview-img" />
              <div v-else class="ref-preview-missing">缺少图片</div>
              <div class="ref-preview-label">{{ ref.label }}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  <div v-else class="studio-empty-page">
    <div class="step-empty">
      <div class="empty-title">这一集暂时打不开</div>
      <div class="empty-desc">{{ loadError || '正在加载项目数据，请稍等后刷新。' }}</div>
      <div class="step-empty-actions">
        <button class="btn" @click="navigateTo(`/drama/${dramaId}`)">返回项目</button>
...
import { dramaAPI, episodeAPI, storyboardAPI, characterAPI, sceneAPI, imageAPI, videoAPI, composeAPI, mergeAPI, gridAPI, aiConfigAPI, voicesAPI, uploadAPI, dashboardAPI } from '~/composables/useApi'
...
const healthData = ref(null), tasksData = ref(null), dashboardLoading = ref(false)
const loadError = ref('')
const panel = ref('script')
const { running: rn, runningType: rt, run: runAgent, runPipeline } = useAgent()
const isGenerating = rn  // one-click generation state mirror
...
const openAIFallbackVoiceProfiles = [
...
const minimaxFallbackVoiceProfiles = [
  { id: 'male-qn-qingse', label: '青涩男声', gender: '男声', traits: '年轻、自然、清亮', suitable: '青年男主、少年感角色、日常对白' },
  { id: 'male-qn-jingying', label: '精英男声', gender: '男声', traits: '稳重、克制、清晰', suitable: '成熟男性、官员、叙事对白' },
  { id: 'male-qn-badao', label: '霸道男声', gender: '男声', traits: '强势、有压迫感', suitable: '反派、权贵、强势人物' },
  { id: 'female-shaonv', label: '少女女声', gender: '女声', traits: '年轻、明亮、柔和', suitable: '少女、年轻女性、受惊对白' },
  { id: 'female-yujie', label: '御姐女声', gender: '女声', traits: '成熟、冷静、有质感', suitable: '成熟女性、权谋角色、冷静对白' },
  { id: 'female-tianmei', label: '甜美女声', gender: '女声', traits: '甜润、亲和、轻快', suitable: '温柔女性、轻松对白、配角' },
]

function fallbackVoiceProfilesFor(provider) {
  return String(provider || '').toLowerCase() === 'minimax'
    ? minimaxFallbackVoiceProfiles
    : openAIFallbackVoiceProfiles
}

const voiceProfiles = ref(fallbackVoiceProfilesFor('minimax'))
const voiceSelectOptions = computed(() => voiceProfiles.value.map(v => ({ label: `${v.label} · ${v.traits}`, value: v.id })))
function configOption(config) {
  let modelName = ''
  try { const m = JSON.parse(config.model || '[]'); modelName = Array.isArray(m) ? (m[0] || '') : (m || '') } catch { modelName = config.model || '' }
  const label = modelName ? `${config.name} · ${modelName} (${config.provider})` : `${config.name} (${config.provider})`
  return { label, value: config.id }
}

const imageConfigSelectOptions = computed(() => imageConfigs.value.map(configOption))
const videoConfigSelectOptions = computed(() => videoConfigs.value.map(configOption))
const audioConfigSelectOptions = computed(() => audioConfigs.value.map(configOption))
...
function closeRefPreview() {
  refPreview.value = { open: false, refs: [] }
}
...
const gridStorageKey = computed(() => `qingqiu:grid:${dramaId}:${epId.value || episodeNumber}`)
...
function prodStepDone(id) {
  ...
  if (id === 'shots') return shotFramesReady.value
...
const firstFrameCount = computed(() => sbs.value.filter(s => getFirstFrame(s)).length)
const lastFrameCount = computed(() => sbs.value.filter(s => getLastFrame(s)).length)
const shotImgCount = computed(() => frameMode.value === 'first_last' ? firstFrameCount.value + lastFrameCount.value : firstFrameCount.value)
const shotFrameTargetCount = computed(() => sbs.value.length * (frameMode.value === 'first_last' ? 2 : 1))
const shotFramesReady = computed(() => {
  if (!sbs.value.length) return false
  if (frameMode.value === 'first_last') return firstFrameCount.value === sbs.value.length && lastFrameCount.value === sbs.value.length
  return firstFrameCount.value === sbs.value.length
})
...
const healthIssueCount = computed(() => (healthData.value?.summary?.errors || 0) + (healthData.value?.summary?.warnings || 0))
const failedTaskCount = computed(() => tasksData.value?.summary?.failed || 0)

const prodTabDefs = computed(() => [
  { id: 'health', label: '项目体检', icon: FileText, badge: healthIssueCount.value ? String(healthIssueCount.value) : '' },
  { id: 'tasks', label: '任务中心', icon: FolderKanban, badge: failedTaskCount.value ? String(failedTaskCount.value) : '' },
  ...
  { id: 'dubbing', label: '配音生成', icon: Mic2, badge: ttsEligibleCount.value ? `${ttsGeneratedCount.value}/${ttsEligibleCount.value}` : '' },
  { id: 'shots', label: '镜头图片', icon: ImageIcon, badge: shotImgCount.value ? `${shotImgCount.value}/${shotFrameTargetCount.value}` : '' },
...
const sidebarSections = computed(() => ([
  ...
  },
  {
    ...
    label: '制作',
    items: [
      { key: 'prod:health', label: '项目体检', desc: '', icon: FileText, done: healthIssueCount.value === 0 && !!healthData.value },
      { key: 'prod:tasks', label: '任务中心', desc: '', icon: FolderKanban, done: failedTaskCount.value === 0 && !!tasksData.value },
...
const activeSubSteps = computed(() => {
  ...
  if (activeMainStage.value === 'assets') {
    return [
      { key: 'prod:health', label: '项目体检', done: healthIssueCount.value === 0 && !!healthData.value },
      { key: 'prod:tasks', label: '任务中心', done: failedTaskCount.value === 0 && !!tasksData.value },
  ...
  if (activeMainStage.value === 'storyboard') {
    return [
      ...
      { key: 'prod:shots', label: '镜头图片', done: shotFramesReady.value },
...
async function refresh() {
  try {
    ...
    if (ep) {
      ...
      loadError.value = `项目里没有找到第 ${episodeNumber} 集。`
    }
  } catch (e) {
...
async function loadDashboard() {
  ...
  try {
    const [health, tasks] = await Promise.all([
      dashboardAPI.health(dramaId, epId.value),
      dashboardAPI.tasks(dramaId, epId.value),
    ])
...
async function saveRaw() { await episodeAPI.update(epId.value, { content: localRaw.value }); episode.value.content = localRaw.value }
...
async function updateEpisodeConfig(field, value) {
  if (!epId.value || !value) return
  try {
    const updated = await episodeAPI.update(epId.value, { [field]: value })
    if (field === 'image_config_id') {
      episode.value.image_config_id = value
      episode.value.imageConfigId = value
    }
    if (field === 'video_config_id') {
      episode.value.video_config_id = value
      episode.value.videoConfigId = value
    }
    if (field === 'audio_config_id') {
      episode.value.audio_config_id = value
      episode.value.audioConfigId = value
    }
    if (updated) episode.value = { ...episode.value, ...updated }
    toast.success('当前集配置已更新')
  } catch (e) {
    toast.error(e.message)
    await refresh()
  }
...
async function doOneClickGenerate() {
  ...
  try {
    ...
    // Step 2: pipeline rewrite + parallel review (story_editor → script_rewriter → compliance + comparative in parallel)
    await runPipeline(
      currentDramaId,
      currentEpId,
...
async function doRewrite() {
  const currentEpId = epId.value
  const currentDramaId = dramaId
  try { await saveRaw() } catch(e) { console.error('saveRaw error:', e) }
  runPipeline(
    currentDramaId,
    currentEpId,
    '请读取原始小说内容，按总编策略改写成格式化短剧剧本。按原文时间顺序自然叙事，不刻意制造强钩子或悬念。',
...
async function batchGenSamples() {
  ...
  const results = await Promise.allSettled(pending.map(c => characterAPI.voiceSample(c.id, currentEpId)))
...
async function watchUntil(check, options = {}) {
  const intervalMs = options.intervalMs ?? 3000
  const maxMs = options.maxMs ?? 30 * 60 * 1000
  const onTimeout = options.onTimeout ?? null
  const refreshBeforeCheck = options.refreshBeforeCheck ?? true

  const start = Date.now()
  const maxAttempts = Math.floor(maxMs / intervalMs)

  for (let i = 0; i < maxAttempts; i++) {
    await sleep(intervalMs)
    if (refreshBeforeCheck) await refresh()
...
async function uploadCharImg(char) {
  try {
    ...
    await characterAPI.update(char.id, { image_url: uploaded.path })
    toast.success('角色标准图已更新')
    await refresh()
  } catch (e) {
    toast.error(e.message)
  }
...
async function uploadSceneImg(scene) {
  try {
    const uploaded = await uploadImageFile()
    if (!uploaded?.path) return
    await sceneAPI.update(scene.id, { image_url: uploaded.path, local_path: uploaded.path })
    toast.success('场景标准图已更新')
    await refresh()
  } catch (e) {
    toast.error(e.message)
  }
...
async function uploadShotFrame(sb, frameType) {
  try {
    const uploaded = await uploadImageFile()
    if (!uploaded?.path) return
    const field = frameType === 'last_frame' ? 'last_frame_image' : 'first_frame_image'
    await storyboardAPI.update(sb.id, { [field]: uploaded.path })
    toast.success(frameType === 'last_frame' ? '尾帧已上传' : '首帧已上传')
    await refresh()
  } catch (e) {
    toast.error(e.message)
  }
...
async function genCharImg(id) {
  const prevImage = chars.value.find(c => c.id === id)?.image_url || chars.value.find(c => c.id === id)?.imageUrl || ''
  try {
...
async function genSceneImg(id) {
  const prevImage = scenes.value.find(s => s.id === id)?.image_url || scenes.value.find(s => s.id === id)?.imageUrl || ''
  try {
...
async function deleteSceneImg(scene) {
  if (!confirm(`确定删除场景"${scene.location}"的图片？`)) return
  await sceneAPI.delImage(scene.id)
  await refresh()
}
...
function getFrameImage(s, frameType) { return frameType === 'last_frame' ? getLastFrame(s) : getFirstFrame(s) }
...
function getLockedImage(item) { return item?.locked_image_url || item?.lockedImageUrl || '' }
function getPrimaryImage(item) { return item?.image_url || item?.imageUrl || getLockedImage(item) || '' }
function getReferencePrompt(item) { return item?.reference_prompt || item?.referencePrompt || '' }

function buildDefaultCharReferencePrompt(char) {
  return [
    char?.name || '角色',
...
function buildDefaultSceneReferencePrompt(scene) {
  return [
    scene?.location || '场景',
    scene?.prompt || '',
    scene?.time ? `时间：${scene.time}` : '',
    '固定场景参考，保持建筑布局、时代质感、光线、色彩和空间关系',
  ].filter(Boolean).join('，')
}

async function saveCharReferencePrompt(char, value) {
  const prompt = String(value || '').trim()
  if (prompt === getReferencePrompt(char)) return
  char.reference_prompt = prompt
  char.referencePrompt = prompt
  try {
    await characterAPI.update(char.id, { reference_prompt: prompt })
    toast.success('角色固定描述已保存')
  } catch (e) {
    toast.error(e.message)
    await refresh()
  }
...
async function saveSceneReferencePrompt(scene, value) {
  const prompt = String(value || '').trim()
  if (prompt === getReferencePrompt(scene)) return
  scene.reference_prompt = prompt
  scene.referencePrompt = prompt
  try {
    await sceneAPI.update(scene.id, { reference_prompt: prompt })
    toast.success('场景固定描述已保存')
  } catch (e) {
    toast.error(e.message)
    await refresh()
  }
...
function getShotReferenceSummary(sb) {
  const items = []
  const sceneId = sb?.scene_id || sb?.sceneId
  const scene = scenes.value.find(item => item.id === sceneId)
  items.push({
    key: `scene:${sceneId || 'none'}`,
    label: scene ? '场景' : '缺场景',
    ok: !!getPrimaryImage(scene),
    tip: scene ? `${scene.location || '场景'}：${getPrimaryImage(scene) || '缺少参考图'}` : '镜头未绑定场景',
  })
  for (const charId of getStoryboardCharacterIds(sb)) {
    const char = chars.value.find(item => item.id === charId)
    items.push({
      key: `char:${charId}`,
      label: `${char?.name || `角色${charId}`}`,
      ok: !!getPrimaryImage(char),
      tip: char ? `${char.name || '角色'}：${getPrimaryImage(char) || '缺少参考图'}` : '角色不存在',
    })
  }
  if (!getStoryboardCharacterIds(sb).length) {
    items.push({ key: 'char:none', label: '无角色', ok: true, tip: '当前镜头未绑定角色' })
  }
  return items
}

function getShotReferenceDetails(sb) {
  const refs = []
  const sceneId = sb?.scene_id || sb?.sceneId
  const scene = scenes.value.find(item => item.id === sceneId)
  refs.push({
    key: `scene:${sceneId || 'none'}`,
    label: scene ? `场景：${scene.location || sceneId}` : '场景：未绑定',
    src: getPrimaryImage(scene),
  })
  for (const charId of getStoryboardCharacterIds(sb)) {
    const char = chars.value.find(item => item.id === charId)
    refs.push({
      key: `char:${charId}`,
      label: `角色：${char?.name || charId}`,
      src: getPrimaryImage(char),
    })
  }
  for (const [index, ref] of getRefs(sb).entries()) {
    refs.push({ key: `manual:${index}`, label: `手动参考 ${index + 1}`, src: ref })
  }
  const first = getFirstFrame(sb)
  const last = getLastFrame(sb)
  if (first) refs.push({ key: 'first', label: '已有首帧', src: first })
  if (last) refs.push({ key: 'last', label: '已有尾帧', src: last })
  return refs
}

function openShotRefs(sb) {
  refPreview.value = { open: true, refs: getShotReferenceDetails(sb) }
}
...
function getShotReferenceImages(sb, frameType = '') {
  ...
  const sceneId = sb?.scene_id || sb?.sceneId
  const scene = scenes.value.find(item => item.id === sceneId)
  pushRef(getPrimaryImage(scene))
  for (const charId of getStoryboardCharacterIds(sb)) {
    const char = chars.value.find(item => item.id === charId)
    pushRef(getPrimaryImage(char))
  }
  ...
  const first = getFirstFrame(sb)
  const last = getLastFrame(sb)
  if (frameType !== 'first_frame') pushRef(first)
  if (frameType !== 'last_frame') pushRef(last)
  return refs.filter(Boolean).slice(0, 6)
}

function getShotReferencePromptLines(sb, frameType = '') {
  const refs = []
  const pushRef = (src, label) => {
    if (!src || refs.some(item => item.src === src) || refs.length >= 6) return
    refs.push({ src, label })
  }
  const sceneId = sb?.scene_id || sb?.sceneId
  const scene = scenes.value.find(item => item.id === sceneId)
  pushRef(getPrimaryImage(scene), scene ? `标准场景：${scene.location || sceneId}` : '标准场景')
  for (const charId of getStoryboardCharacterIds(sb)) {
    const char = chars.value.find(item => item.id === charId)
    pushRef(getPrimaryImage(char), `标准角色：${char?.name || charId}`)
  }
  for (const [index, ref] of getRefs(sb).entries()) {
    pushRef(ref, `手动参考：${index + 1}`)
  }
  if (frameType !== 'first_frame') pushRef(getFirstFrame(sb), '已有首帧')
  if (frameType !== 'last_frame') pushRef(getLastFrame(sb), '已有尾帧')
  return refs.map((item, index) => `参考图${index + 1}=${item.label}`)
}

function compactPromptText(value, max = 180) {
  ...
  const safeCut = boundary >= Math.floor(max * 0.55) ? cut.slice(0, boundary + 1) : cut
  return `${safeCut.trim()}...`
}

function getShotFixedPromptText(sb) {
  const characterIds = getStoryboardCharacterIds(sb)
  const shotCharacters = chars.value.filter(char => characterIds.includes(char.id))
  const scene = scenes.value.find(item => item.id === (sb?.scene_id || sb?.sceneId))
  const sceneRefPrompt = scene ? compactPromptText(getReferencePrompt(scene) || buildDefaultSceneReferencePrompt(scene), 260) : ''
  const characterRefPrompts = shotCharacters
    .map(char => `${char.name}：${compactPromptText(getReferencePrompt(char) || buildDefaultCharReferencePrompt(char), 240)}`)
    .filter(Boolean)
  return [
    characterRefPrompts.length ? `【角色固定描述】\n${characterRefPrompts.join('\n')}` : '',
    sceneRefPrompt ? `【场景固定描述】\n${scene?.location || '场景'}：${sceneRefPrompt}` : '',
  ].filter(Boolean).join('\n\n')
}
...
function buildShotImagePrompt(sb, frameType) {
  ...
  const fixedPromptText = getShotFixedPromptText(sb)
  const referencePromptLines = getShotReferencePromptLines(sb, frameType)
  ...
  const styleHint = `${drama.value?.style || 'anime'} style`

  const fixedRules = [
    referencePromptLines.length ? `参考图映射：${referencePromptLines.join('；')}` : '',
    fixedPromptText,
    '严格沿用参考图身份、脸型、发型、服装、身高比例、场景布局和时代质感；不要重新设计角色，不要更换服装，不要改变场景结构。',
  ].filter(Boolean).join('\n')
  ...
  const shotRules = [
    ...
    charactersText ? `本镜头角色：${charactersText}` : '',
    location ? `本镜头地点：${location}` : '',
    time ? `时间：${time}` : '',
    action ? `动作：${action}` : '',
...
function applyImageRecordToLocal(record) {
  const storyboardId = record?.storyboard_id || record?.storyboardId
  const frameType = record?.frame_type || record?.frameType
  const localPath = record?.local_path || record?.localPath || record?.image_url || record?.imageUrl
  if (!storyboardId || !frameType || !localPath) return false

  const target = sbs.value.find(s => s.id === storyboardId)
  if (!target) return false

  if (frameType === 'first_frame') {
    target.first_frame_image = localPath
    target.firstFrameImage = localPath
  } else if (frameType === 'last_frame') {
    target.last_frame_image = localPath
    target.lastFrameImage = localPath
  } else {
    target.composed_image = localPath
    target.composedImage = localPath
  }
  return true
}

async function pollShotFrameGeneration(generationId, job, options = {}) {
  const key = framePendingKey(job.sb.id, job.frameType)
  await watchUntil(async () => {
    let record
    try {
      record = await imageAPI.get(generationId)
    } catch {
      return false
    }
    if (record?.status === 'completed') {
      applyImageRecordToLocal(record)
      pendingShotFrameKeys.value = pendingShotFrameKeys.value.filter(item => item !== key)
      if (options.toast !== false) toast.success(job.frameType === 'first_frame' ? '首帧生成完成' : '尾帧生成完成')
      return true
    }
    if (record?.status === 'failed') {
      pendingShotFrameKeys.value = pendingShotFrameKeys.value.filter(item => item !== key)
      if (options.toast !== false) toast.error(record?.error_msg || record?.errorMsg || '帧图生成失败')
      return true
    }
    return false
  }, { intervalMs: 3000, maxMs: 30 * 60 * 1000, refreshBeforeCheck: false, onTimeout: () => {
    pendingShotFrameKeys.value = pendingShotFrameKeys.value.filter(item => item !== key)
    if (options.toast !== false) toast.error('帧图生成查询超时')
  }})
}

async function genShotFrame(sb, frameType, options = {}) {
  const prevImage = getFrameImage(sb, frameType)
  const prompt = buildShotImagePrompt(sb, frameType)
  const referenceImages = getShotReferenceImages(sb, frameType)
  const missingRefs = getShotReferenceSummary(sb).filter(item => !item.ok)
  if (missingRefs.length && !options.silent) {
    toast.warning(`缺少参考图：${missingRefs.map(item => item.label).join('、')}，将按现有参考继续生成`)
  }
  ...
  try {
    ...
    if (!options.silent) toast.success(frameType === 'first_frame' ? '首帧生成中' : '尾帧生成中')
    if (options.watchResult !== false) {
      if (generation?.id) {
...
function getShotFrameBatchJobs(kind = 'missing') {
  const includeFirst = kind === 'first' || kind === 'missing'
  const includeLast = kind === 'last' || (kind === 'missing' && frameMode.value === 'first_last')
  const jobs = []
  for (const sb of sbs.value) {
    if (includeFirst && !getFirstFrame(sb) && !isPendingShotFrame(sb.id, 'first_frame')) {
      jobs.push({ sb, frameType: 'first_frame', prevImage: null })
    }
    if (includeLast && !getLastFrame(sb) && !isPendingShotFrame(sb.id, 'last_frame')) {
      jobs.push({ sb, frameType: 'last_frame', prevImage: null })
    }
  }
  return jobs
}

function watchBatchShotFrames(jobs) {
  const pendingJobs = new Map(
    jobs
      .filter(job => job.generationId)
      .map(job => [job.generationId, job]),
  )
  if (!pendingJobs.size) return

  void watchUntil(async () => {
    let rows = []
    try {
      rows = await imageAPI.list({ drama_id: dramaId })
    } catch {
      return false
    }
    ...
    // timeout: clear remaining pending keys for these jobs
    jobs.forEach(job => {
      const key = framePendingKey(job.sb.id, job.frameType)
      pendingShotFrameKeys.value = pendingShotFrameKeys.value.filter(item => item !== key)
    })
...
async function batchGenerateShotFrames(kind = 'missing') {
  const jobs = getShotFrameBatchJobs(kind).map(job => ({ ...job, prevImage: getFrameImage(job.sb, job.frameType) }))
  if (!jobs.length) {
    toast.info(kind === 'last' ? '尾帧都已经有了' : kind === 'first' ? '首帧都已经有了' : '当前模式下没有缺失帧图')
    return
  }
  pendingBatchShotFrames.value = true
  let okCount = 0
  let failCount = 0
  const submittedJobs = []
  try {
    for (const job of jobs) {
      try {
        const generationId = await genShotFrame(job.sb, job.frameType, { silent: true, skipRefresh: true, watchResult: false })
        okCount++
        submittedJobs.push({ ...job, generationId })
        await sleep(350)
      } catch {
        failCount++
      }
    ...
    if (okCount) {
      watchBatchShotFrames(submittedJobs)
      toast.success(`已提交 ${okCount} 个镜头帧图任务`)
    }
    if (failCount) toast.error(`${failCount} 个帧图任务提交失败`)
  } finally {
    pendingBatchShotFrames.value = false
  }
}

const SAFE_VIDEO_REPLACEMENTS = [
  [/吐血|流血|血腥|血/g, '受伤'],
  [/打晕|打死|杀死|杀/g, '制服'],
  [/绑架|捆绑|绑/g, '控制住'],
  [/审问|逼问/g, '询问'],
  [/暴力|殴打|猛打|打得毫无还手之力/g, '激烈对抗'],
  [/后宫秘闻|有染/g, '宫廷传闻'],
]

function buildSafeVideoPrompt(raw) {
  let prompt = String(raw || '')
  for (const [pattern, replacement] of SAFE_VIDEO_REPLACEMENTS) {
    prompt = prompt.replace(pattern, replacement)
  }
  return [
    prompt,
    '画面表现克制，不出现血腥、伤口、酷刑、捆绑细节或过度暴力，只表现动作结果和人物情绪。',
  ].filter(Boolean).join('；')
}

function buildSilentVideoPrompt(raw) {
  const prompt = String(raw || '').trim()
  // 让角色自然说话表演，口型由视频模型自行决定
  const lipSyncRule = 'character speaking, visible mouth movement, natural facial expressions, emotional acting'
  return prompt ? [prompt, lipSyncRule].filter(Boolean).join('；') : lipSyncRule
}

function applyVideoGenerationToLocal(record, storyboardId) {
  const targetId = storyboardId || record?.storyboard_id || record?.storyboardId
  const localPath = record?.local_path || record?.localPath || record?.video_url || record?.videoUrl
  if (!targetId || !localPath) return false
  const target = sbs.value.find(s => s.id === targetId)
  if (!target) return false
  target.video_url = localPath
  target.videoUrl = localPath
  if (record?.duration) target.duration = record.duration
  return true
}

function applyComposeStatusItemsToLocal(items = []) {
  for (const item of items) {
    const id = item?.id
    const target = sbs.value.find(s => s.id === id)
    if (!target) continue
    const composedUrl = item.composed_video_url || item.composedVideoUrl
    if (composedUrl) {
      target.composed_video_url = composedUrl
      target.composedVideoUrl = composedUrl
    }
    if (item.status) target.status = item.status
  }
}

async function genVid(sb, options = {}) {
  const fixedPromptText = getShotFixedPromptText(sb)
  const basePrompt = options.safeRewrite
    ? buildSafeVideoPrompt(sb.video_prompt || sb.videoPrompt || sb.description || '')
    : (sb.video_prompt || sb.videoPrompt || '')
  const prompt = [buildSilentVideoPrompt(basePrompt), fixedPromptText].filter(Boolean).join('；')
  const params = {
    storyboard_id: sb.id,
    drama_id: dramaId,
    prompt,
  ...
  const first = getFirstFrame(sb)
  const last = getLastFrame(sb)
  const refs = getShotReferenceImages(sb, 'video').filter(ref => ref !== first && ref !== last)
  ...
  try {
    ...
    toast.success(options.safeRewrite ? '安全版视频生成中' : '视频生成中')
...
function retrySafeVideo(sb) {
  return genVid(sb, { safeRewrite: true })
}
  ...
  await watchUntil(async () => {
    try {
      ...
      if (res?.status === 'completed') {
        applyVideoGenerationToLocal(res, storyboardId)
        pendingVideoIds.value = pendingVideoIds.value.filter(item => item !== storyboardId)
    ...
    return false
  }, { intervalMs: 4000, maxMs: 8 * 60 * 1000, refreshBeforeCheck: false, onTimeout: () => {
    pendingVideoIds.value = pendingVideoIds.value.filter(item => item !== storyboardId)
    failedVideoMessages.value = {
      ...failedVideoMessages.value,
      [storyboardId]: '视频生成超时',
...
async function doMerge() {
  ...
  await watchUntil(async () => {
    let data
    try {
      data = await mergeAPI.status(epId.value)
      mergeData.value = data
    } catch {
      return false
    }
    return data?.status === 'completed' || data?.status === 'failed'
  }, { intervalMs: 3000, maxMs: 30 * 60 * 1000, refreshBeforeCheck: false, onTimeout: null })
  await refresh()
  if (mergeData.value?.status === 'completed') toast.success('拼接完成')
  else if (mergeData.value?.status === 'failed') toast.error('拼接失败')
}
...
async function pollComposeStatus() {
  await watchUntil(async () => {
    let res
    try {
      res = await composeAPI.status(epId.value)
      composeData.value = res
    } catch {
      return false
    }

    const items = Array.isArray(res?.items) ? res.items : []
    applyComposeStatusItemsToLocal(items)
    ...
    toast.error('批量合成查询超时，请稍后刷新查看结果')
  }})
}
function getRefs(sb) {
...
.health-row.is-error { border-color: rgba(239, 68, 68, 0.25); background: rgba(239, 68, 68, 0.06); }
.health-row.is-warning { border-color: rgba(245, 158, 11, 0.25); background: rgba(245, 158, 11, 0.06); }
.health-row.is-ok { border-color: rgba(34, 197, 94, 0.25); background: rgba(34, 197, 94, 0.06); }
.health-level {
  width: 42px;
  flex-shrink: 0;
  font-size: 11px;
...
.frame-ref-strip { display: flex; flex-wrap: wrap; gap: 4px; }
.frame-ref-chip {
  font-size: 10px;
  line-height: 1;
  padding: 4px 6px;
...
.frame-upload-btn:hover { color: var(--accent); }
.frame-thumb {
