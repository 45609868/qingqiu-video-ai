...
export const sceneAPI = {
  ...
  delImage: (id: number) => api.del(`/scenes/${id}/image`),
}

export const uploadAPI = {
  image: (file: File) => uploadReq('/upload/image', file),
}

export const dashboardAPI = {
  health: (dramaId: number, episodeId?: number) => {
    const query = episodeId ? `?episode_id=${episodeId}` : ''
    return api.get(`/dashboard/dramas/${dramaId}/health${query}`)
  },
  tasks: (dramaId: number, episodeId?: number) => {
    const query = episodeId ? `?episode_id=${episodeId}` : ''
    return api.get(`/dashboard/dramas/${dramaId}/tasks${query}`)
  },
}
